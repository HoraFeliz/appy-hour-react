...
export default function ImageCanvas({ place, recommended, brand, placeInfo, tour, id, loading, touch, first }) {
		...
		if (recommended) {
			return (
				<div className={`appy--image-tours-recommended ${brand}`}>
					{brand === 'mahou' ? (
						<img src="../../img/logo-mahou.svg" alt="Mahou" />
					) : brand === 'estrella' ? (
						<img src="../../img/logo-estrella.svg" alt="Estrella" />
					) : brand === 'recommended' ? (
						<FontAwesomeIcon icon={faStar} />
					) : null}
				</div>
	...
	return loading ? (
		<div
			className={`appy--image-placeid appy--image-placeid-loading appy--image-loading loading--background-default 
                ${touch && 'loading--background-default-touch'} ${!first && 'loading--background-default-delay4'}`}
			style={{ marginRight: '5px', height: '100px' }}
		>
			<div className="appy--image-tours-recommended appy--image-tours-recommended-loading" />
			<BeerRating loading={true} />
		</div>
	) : (
		<div
			id="place-image"
			onClick={placeInfo && handleZoom}
			className={place ? 'appy--image-placeid' : 'appy--tours-item-image'}
			style={{
				backgroundImage: `url('${placeInfo ? placeInfo.photo : tour ? tour[id].image : null}')`
			}}
		>
			{Recommended()}

			<BeerRating place={place} rating={placeInfo ? Math.round(placeInfo.rating) : 4} />
			{!tour && <ImageZoom zoom={localStorage.getItem('imageZoom')} />}
		</div>
	);
}
