...
export const getIsPlaceOpen = (id) => {
	return placesHttp.get(`?placeId=${id}&key=${process.env.REACT_APP_MAPS_API_KEY}`).then((response) => {
...
export const savePlace = (body, tourId) => {
	return http.post(`/place/new/${tourId}`, body).then((response) => response.data);
};
